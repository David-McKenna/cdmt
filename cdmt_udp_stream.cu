...
int main(int argc,char *argv[])
{
  ...
  int nstream = 2 + ndm;
  nstream = (nstream>16) ? 16 : nstream; // most CUDA devices can only support 16 streams; cap as a result.

  cudaStream_t stream[nstream];
  cudaEvent_t copyEvent[2];
  cudaEvent_t fftEvent[2];
  cudaEvent_t dmEventInit[nstream - 2];
  cudaEvent_t dmEventFin[nstream - 2];


  for (i = 0; i < nstream; i++) cudaStreamCreate(&(stream[i]));
  
  for (i = 0; i < 2; i++) {
    cudaEventCreate(&(copyEvent[i]));
    cudaEventCreate(&(fftEvent[i]));
  }
  
  for (i = 0; i < nstream - 2; i++) {
    cudaEventCreate(&(dmEventInit[i]));
    cudaEventCreate(&(dmEventFin[i]));
  }

  char *udpbuf[2][4]; // 2 Ingest streams
  unsigned char *cbuf[2][ndm];
  float *cbuff[2][ndm];
  ...
  cufftSetStream(ftc2cf, stream[0]);
  ...
  cufftSetStream(ftc2cb, stream[0]);
  cudaDeviceSynchronize();
  ...
  for (i=0;i<4;i++) {
    for (int j = 0; j < 2; j++) udpbuf[j][i]= reader->meta->outputData[i];
  ...
  if (redig) {
    for (i = 0; i < ndm; i++) for (int j = 0; j < 2; j++) cbuf[j][i]=(unsigned char *) malloc(sizeof(unsigned char)*msamp*mchan/ndec);
    checkCudaErrors(cudaMalloc((void **) &dcbuf, (size_t) sizeof(unsigned char)*msamp*mchan/ndec));
  }
  else {
    for (i = 0; i < ndm; i++) for (int j = 0; j < 2; j++) cbuff[j][i] = (float *) malloc(sizeof(float)*msamp*mchan/ndec);
  ...
  */
  ...
  #pragma omp parallel for ordered shared(nread,total_ts_read,timeInSeconds,breakVar) private(startclock,nread_tmp,idm,i) schedule(static,1)
  for (iblock=0;iblock < 1000000;iblock++) {
    #pragma omp cancellation point for
    if (breakVar) continue;
    // Read block
    int streamId = omp_get_thread_num();
    int refId = (streamId + 1) % 2;
    #pragma omp ordered 
    {
      startclock=clock();
      ...
      cudaStreamWaitEvent(stream[streamId], copyEvent[refId], 0);
      // Copy buffers to device
      startclock=clock();
      for (i=0;i<4;i++)
        checkCudaErrors(cudaMemcpyAsync(dudpbuf[i],udpbuf[streamId][i],sizeof(char)*nread*nsub,cudaMemcpyHostToDevice,stream[streamId]));
    ...
    cudaStreamWaitEvent(stream[streamId], fftEvent[refId], 0);
    // Unpack data and padd data
    blocksize.x=32; blocksize.y=32; blocksize.z=1;
    gridsize.x=nbin/blocksize.x+1; gridsize.y=nfft/blocksize.y+1; gridsize.z=nsub/blocksize.z+1;
    unpack_and_padd<<<gridsize,blocksize,0,stream[streamId]>>>(dudpbuf[0],dudpbuf[1],dudpbuf[2],dudpbuf[3],nread,nbin,nfft,nsub,noverlap,cp1p,cp2p);


    cudaEventRecord(copyEvent[streamId], stream[streamId]);
    cufftSetStream(ftc2cf, stream[streamId]);
    // Perform FFTs
    ...
    swap_spectrum_halves<<<gridsize,blocksize,0,stream[streamId]>>>(cp1p,cp2p,nbin,nfft*nsub);

    cudaStreamSynchronize(stream[streamId]);
    // Loop over dms
    for (idm=0;idm<ndm;idm++) {
      int dmStream = (idm % (nstream - 2)) + 2;
      int dmEventRef = ((idm + 1) % (nstream - 2)) + 2;
      if (idm == 0) dmEventRef = ((ndm) % (nstream - 2)) + 2; // Get the event from the last time iteration
      
      cudaStreamWaitEvent(stream[dmStream], dmEventFin[dmEventRef], 0);
      // dm init
      ...
      PointwiseComplexMultiply<<<gridsize,blocksize,0,stream[dmStream]>>>(cp1p,dc,cp1,nbin*nsub,nfft,idm,1.0/(float) nbin);
      PointwiseComplexMultiply<<<gridsize,blocksize,0,stream[dmStream]>>>(cp2p,dc,cp2,nbin*nsub,nfft,idm,1.0/(float) nbin);

      // if dm == -1, fftFin event
      if(idm == (ndm -1)) cudaEventRecord(fftEvent[streamId], stream[dmStream]);
      
      // Swap spectrum halves for small FFTs
      blocksize.x=32; blocksize.y=32; blocksize.z=1;
      gridsize.x=mbin/blocksize.x+1; gridsize.y=nchan*nfft*nsub/blocksize.y+1; gridsize.z=1;
      swap_spectrum_halves<<<gridsize,blocksize,0,stream[dmStream]>>>(cp1,cp2,mbin,nchan*nfft*nsub);
      
      // Perform FFTs
      cufftSetStream(ftc2cb, stream[dmStream]);
      ...
      transpose_unpadd_and_detect<<<gridsize,blocksize,0,stream[dmStream]>>>(cp1,cp2,mbin,nchan,nfft,nsub,noverlap/nchan,nread/nchan,dfbuf);
    ...
    decimate_and_redigitize<<<gridsize,blocksize>>>(dfbuf,ndec,mchan,mblock,msum,zavg,zstd,3.0,5.0,dcbuf);      
        ...
        // Copy buffer to host
        checkCudaErrors(cudaMemcpyAsync(cbuf[streamId][idm],dcbuf,sizeof(unsigned char)*msamp*mchan/ndec,cudaMemcpyDeviceToHost,stream[dmStream]));
        // dm fin
        // Write buffer

      } else {
        if (ndec==1) checkCudaErrors(cudaMemcpyAsync(cbuff[streamId][idm], dfbuf,sizeof(float)*msamp*mchan,cudaMemcpyDeviceToHost,stream[dmStream]));
        
        else {
          blocksize.x=32; blocksize.y=32; blocksize.z=1;
          gridsize.x=mchan/blocksize.x+1; gridsize.y=mblock/blocksize.y+1; gridsize.z=1;
          decimate<<<gridsize,blocksize>>>(dfbuf,ndec,mchan,mblock,msum,dcbuff);
          checkCudaErrors(cudaMemcpyAsync(cbuff[idm],dcbuff,sizeof(float)*msamp*mchan/ndec,cudaMemcpyDeviceToHost));
          // dm fin
        }
      ...
      cudaEventRecord(dmEventFin[dmStream], stream[dmStream]);
      // Write buffer
    }

    cudaStreamSynchronize(stream[((ndm) % (nstream - 2)) + 2]);
    
    #pragma omp ordered 
    {
      for (idm = 0; idm < ndm; idm++) {
        if (redig) fwrite(cbuf[streamId][idm],sizeof(char),nread*nsub/ndec,outfile[idm]);
        else fwrite(cbuff[streamId][idm],sizeof(float),nread*nsub/ndec, outfile[idm]);
      }
    }
